(function (global, undefined) {
	...
	var Terminal = Terminal || function(containerID, options) {
		...
		function processNewCommand(e, manual) {
		...
		function output(html) {
			...
			if( typeof html.then == 'function') {
				html.then(function(content){
					_output.insertAdjacentHTML('beforeEnd', content);
					_cmdLine.scrollIntoView();
				})
			}else{
				if(html.nodeName){
				_output.appendChild(html)
				}else{
		...
		function refresh(){
			var elements = document.getElementsByTagName("cmd");
	        for (var i=0; i < elements.length; i++)
	        {
	            ...
	            elements[i].addEventListener('click', function(e){
	                var line = document.getElementsByClassName("cmdline");
					var cmd =this.dataset.cmd
	                line[0].value = cmd

	                terminal.run()
	                //console.log(e.target.dataset.cmd)
	            })
	        }
		}
		...
		return {
			...
			run: function(){
                processNewCommand.apply(document.getElementsByClassName("cmdline")[0], [{ keyCode: 13 }, true])
                _cmdLine.focus();
            },
            refresh: refresh
        }
